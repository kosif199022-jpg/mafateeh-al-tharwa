...
required=(
  ...
  public/reader-master-fixes.js public/reader-audio-touch-guard.js public/piper-worker.js public/sw.js public/audio/manifest.json tests/scroll-regressions.test.mjs
...
# Mobile audio modal: dedicated window-capture guard owns vertical gestures before legacy handlers.
node tests/audio-sheet-scroll-v36-regression.test.mjs
grep -q 'id="mkAudioSheetScrollV36"' public/reader.html || fail 'Audio sheet scroll CSS was not injected'
grep -q 'reader-audio-touch-guard.js?v=36.4' public/reader.html || fail 'Dedicated audio touch guard is not loaded'
grep -q "window.addEventListener('touchmove',move,{capture:true,passive:false})" public/reader-audio-touch-guard.js || fail 'Window-level touch interception is missing'
grep -q 'stopImmediatePropagation' public/reader-audio-touch-guard.js || fail 'Legacy page touch handlers can still receive audio swipes'
grep -q 'mafateeh-runtime-v35-audio-touch-guard-4' public/sw.js || fail 'Audio touch-guard cache bust is missing'

echo "Master regression guard passed: v$version, 34 MP3s, $bytes bytes, advanced reader, AI, Piper, offline runtime, distraction-free UI and dedicated mobile audio touch guard present."
